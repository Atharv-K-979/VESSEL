    ...
    async initialize() {
        ...
        try {
            if (!this.generator) {
                ...
                // Allow fetching models from the Hugging Face Hub
                env.backends.onnx.wasm.wasmPaths = chrome.runtime.getURL('lib/');

                console.log('Initializing local generator model... This may take a moment.');
                this.generator = await pipeline('text2text-generation', 'Xenova/LaMini-FLAN-T5-248M', {
                    progress_callback: (x) => {
                        // Could dispatch an event or log progress here
                        if (x.status === 'done') console.log(`Loaded ${x.file}`);
                    }
                });
                console.log('Local generator initialized successfully.');
            }
        } catch (error) {
            console.error('Failed to initialize local generator:', error);
            this.generator = null;
        }
