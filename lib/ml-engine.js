...
class MLEngine {
    constructor() {
        ...
        // Explicitly set WASM paths for Chrome Extension environment
        ort.env.wasm.wasmPaths = chrome.runtime.getURL('lib/');
        ort.env.wasm.numThreads = 1; // Prevent ServiceWorker from trying to spawn Web Workers
    ...
    async extractFeatures(text) {
        ...
        // Trained model now uses ONLY technical indicators (dim=18)
    ...
    async detectInjection(text) {
        ...
        // If 'onnx' backend was truly implemented for injection, we'd invoke it here.
        // For now, we still use the heuristic as a placeholder.
        for (let p of patterns) {
            if (lower.includes(p)) return 0.9;
        }
        return 0.1;
