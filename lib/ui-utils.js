...
export function createRequirementsModal(requirements, onInject, isConfigured = true) {
  ...
  shadow.querySelector('.accept-all-button').addEventListener('click', () => {
    onInject(parsedRequirements.map(r => r.description));
    container.remove();
  });
...
export function showThreatModal(score, originalText, sanitizedText, onProceed, onSendSanitized, onCancel) {
  const { container, shadow } = createShadowModal();
  ...
  overlay.innerHTML = `
    <div class="modal-content" style="max-width: 600px; width: 90%;">
      ...
      <p><strong>Threat Score:</strong> <span style="color: #DC2626; font-weight: bold;">${displayScore}/10</span></p>
      <p style="color: #4B5563; margin-bottom: 16px;">This page contains hidden text or instructions that may manipulate the AI assistant.</p>
  ...
  toggleBtn.addEventListener('click', () => {
    ...
    if (viewingSanitized) {
      ...
      sendSanitizedBtn.style.display = 'block'; // Show Send Sanitized button when viewing clean text
    } else {
      ...
      sendSanitizedBtn.style.display = 'none'; // Hide it when raw payload is visible
    }
  });
